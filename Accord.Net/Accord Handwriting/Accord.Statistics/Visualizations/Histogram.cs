...
namespace Accord.Statistics.Visualizations
{
    ...
    public class Histogram
    {
        ...
        {
            get { return this.m_cumulative; }
            set { this.m_cumulative = value; }
        }
        #endregion
        ...
        {
            this.m_range = Matrix.Range(data);
        ...
        private void compute(double[] data)
        {
            ...
            if (m_cumulative)
            {
                for (int i = 1; i < this.m_binCount; i++)
                    m_binValues[i] += m_binValues[i - 1];
            }
        }
        #endregion
