...
namespace Accord.Controls
{
	...
	public class ArrayRowView : ICustomTypeDescriptor, IEditableObject, IDataErrorInfo
	{
		...
		{
            if (_owner.Data.Rank == 2)
            {
                return _owner.Data.GetValue(_index, index);
            }
            else
            {
                return _owner.Data.GetValue(index);
            }
		}
		...
		internal void SetColumnValue(int index,object value)
		{
			try
			{
				_owner.Data.SetValue(value,_index,index);
			}
			catch(Exception e)
			{
				_error = e.ToString();
		...
		public PropertyDescriptorCollection GetProperties(Attribute[] attributes)
		{
            if (_owner.Data.Rank == 2)
            {
                int col = _owner.Data.GetLength(1);
                Type type = _owner.Data.GetType().GetElementType();
            ...
            {
                int col = _owner.Data.GetLength(0);
                Type type = _owner.Data.GetType().GetElementType();
                PropertyDescriptor[] prop = new PropertyDescriptor[col];
                for (int i = 0; i < col; i++)
                {
